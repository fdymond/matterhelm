/**
 * Specification tests for the IPC protocol (docs/BLUEPRINT.md §2.3).
 *
...
describe("hello frame", () => {
  const valid = { v: 1, type: "hello", token: "session-token", protocol: 1 };
  ...
  it("rejects a v field from a future revision", () => {
    expect(HelloFrameSchema.safeParse({ ...valid, v: 2 }).success).toBe(false);
  });
  ...
  it("rejects a protocol field other than 1", () => {
...
describe("action frame — bare actions (no payload)", () => {
  ...
  it.each(bareNames)("accepts a well-formed %s action", (name) => {
    const frame = { v: 1, type: "action", id: uuid1, name };
  ...
  it.each(bareNames)("rejects %s carrying an unexpected value field", (name) => {
    const frame = { v: 1, type: "action", id: uuid1, name, value: 1 };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(false);
  });

  it("rejects an unknown action name", () => {
    const frame = { v: 1, type: "action", id: uuid1, name: "rewind" };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(false);
  });

  it("rejects a malformed uuid in id", () => {
    const frame = { v: 1, type: "action", id: "not-a-uuid", name: "playPause" };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(false);
  });

  it("rejects a missing id", () => {
    const frame = { v: 1, type: "action", name: "playPause" };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(false);
  });

  it("rejects a wrong v field", () => {
    const frame = { v: 2, type: "action", id: uuid1, name: "playPause" };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(false);
  });

  it("rejects unknown extra keys (strict boundary)", () => {
    const frame = { v: 1, type: "action", id: uuid1, name: "playPause", extra: "nope" };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(false);
  });
});
...
describe("action frame — setVolume", () => {
  it("accepts a well-formed setVolume action", () => {
    const frame = { v: 1, type: "action", id: uuid1, name: "setVolume", value: 40 };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(true);
  });

  it("accepts the boundary value 0", () => {
    const frame = { v: 1, type: "action", id: uuid1, name: "setVolume", value: 0 };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(true);
  });

  it("accepts the boundary value 100", () => {
    const frame = { v: 1, type: "action", id: uuid1, name: "setVolume", value: 100 };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(true);
  });

  it("rejects a missing value field", () => {
    const frame = { v: 1, type: "action", id: uuid1, name: "setVolume" };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(false);
  });

  it("rejects volume below 0", () => {
    const frame = { v: 1, type: "action", id: uuid1, name: "setVolume", value: -1 };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(false);
  });

  it("rejects volume above 100", () => {
    const frame = { v: 1, type: "action", id: uuid1, name: "setVolume", value: 101 };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(false);
  });

  it("rejects a fractional volume", () => {
    const frame = { v: 1, type: "action", id: uuid1, name: "setVolume", value: 100.5 };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(false);
  });
...
describe("action frame — setMuted", () => {
  it("accepts a well-formed setMuted action (true)", () => {
    const frame = { v: 1, type: "action", id: uuid1, name: "setMuted", value: true };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(true);
  });

  it("accepts a well-formed setMuted action (false)", () => {
    const frame = { v: 1, type: "action", id: uuid1, name: "setMuted", value: false };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(true);
  });

  it("rejects a missing value field", () => {
    const frame = { v: 1, type: "action", id: uuid1, name: "setMuted" };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(false);
  });
  ...
  it("rejects a non-boolean value (0/1 number)", () => {
    const frame = { v: 1, type: "action", id: uuid1, name: "setMuted", value: 1 };
    expect(ActionFrameSchema.safeParse(frame).success).toBe(false);
  });
});
...
describe("pairing frame", () => {
  ...
  it("rejects a missing qrPayload", () => {
    const rest = { v: 1, type: "pairing", manualCode: "3497-011-2332" };
...
describe("ack frame", () => {
  ...
  it("rejects ok:true carrying an error field", () => {
    const frame = { v: 1, type: "ack", id: uuid1, ok: true, error: "should not be here" };
    expect(AckFrameSchema.safeParse(frame).success).toBe(false);
  });

  it("accepts a well-formed ok:false ack with an error string", () => {
    const frame = { v: 1, type: "ack", id: uuid1, ok: false, error: "socket disconnected" };
    expect(AckFrameSchema.safeParse(frame).success).toBe(true);
  });

  it("accepts ok:false with no error field (error is optional)", () => {
    const frame = { v: 1, type: "ack", id: uuid1, ok: false };
    expect(AckFrameSchema.safeParse(frame).success).toBe(true);
  });
  ...
  it("rejects a malformed uuid in id", () => {
    const frame = { v: 1, type: "ack", id: "not-a-uuid", ok: true };
    expect(AckFrameSchema.safeParse(frame).success).toBe(false);
  });

  it("rejects unknown extra keys (strict boundary)", () => {
    const frame = { v: 1, type: "ack", id: uuid1, ok: true, extra: "nope" };
    expect(AckFrameSchema.safeParse(frame).success).toBe(false);
  });
});

describe("state frame", () => {
  const valid = { v: 1, type: "state", volume: 40, muted: false };
...
describe("parseSidecarFrame (trust boundary, sidecar -> tray)", () => {
  it("accepts a hello frame", () => {
    const result = parseSidecarFrame({ v: 1, type: "hello", token: "t", protocol: 1 });
  ...
  it("rejects a tray-only frame type (state)", () => {
    const result = parseSidecarFrame({ v: 1, type: "state", volume: 40, muted: false });
...
describe("parseTrayFrame (trust boundary, tray -> sidecar)", () => {
  it("accepts an ack frame", () => {
    const result = parseTrayFrame({ v: 1, type: "ack", id: uuid2, ok: true });
    expect(result.success).toBe(true);
  });

  it("accepts a state frame", () => {
    const result = parseTrayFrame({ v: 1, type: "state", volume: 12, muted: true });
    expect(result.success).toBe(true);
  });

  it("rejects a sidecar-only frame type (hello)", () => {
    const result = parseTrayFrame({ v: 1, type: "hello", token: "t", protocol: 1 });
    expect(result.success).toBe(false);
  });
...
describe("SidecarFrameSchema / TrayFrameSchema (direct schema symmetry)", () => {
  it("SidecarFrameSchema rejects frames only valid on the tray side", () => {
    expect(SidecarFrameSchema.safeParse({ v: 1, type: "ack", id: uuid1, ok: true }).success).toBe(
      false,
    );
  });
  ...
  it("TrayFrameSchema rejects frames only valid on the sidecar side", () => {
    expect(
      TrayFrameSchema.safeParse({ v: 1, type: "action", id: uuid1, name: "playPause" }).success,
    ).toBe(false);
  });
});
