...
public sealed class SettingsViewModelTests : IDisposable
{
    ...
    public void SavedDetectedAdapterHiddenByDefaultRemainsVisibleAndExplainsWhy()
    {
        ...
        Assert.Equal(
            ("vEthernet (WSL)", "vEthernet (WSL) — 172.20.16.1 (hidden by filter)"),
            vm.GetMdnsInterfaceChoices()[1]);
        Assert.Equal(
            ("vEthernet (WSL)", "vEthernet (WSL) — 172.20.16.1"),
    ...
    public void ShowAllFallbackRestoresThePreviousInclusiveAdapterList()
    {
        SettingsViewModel vm = NewViewModel(
            networkAdapters: new StubNetworkAdapterProvider(
            [
                new NetworkAdapterInfo("Ethernet", "192.168.1.20", "Realtek PCIe GbE"),
                ...
                new NetworkAdapterInfo("Disconnected", "10.0.0.2", "USB Ethernet", OperationalStatus.Down),
            ]));

        Assert.Equal(
            ["Auto (recommended)", "Ethernet — 192.168.1.20"],
            vm.GetMdnsInterfaceChoices().Select(choice => choice.Label));
        Assert.Equal(
            ["Auto (recommended)", "Ethernet — 192.168.1.20", "Work VPN — no IPv4"],
    ...
    public void ShowAllCheckboxIsUncheckedUiOnlyAndRefreshesTheDropdown()
    {
        SettingsViewModel vm = NewViewModel(
            ...
            [
                new NetworkAdapterInfo("Ethernet", "192.168.1.20", "Realtek PCIe GbE"),
                new NetworkAdapterInfo("Work VPN", "10.8.0.2", "TAP-Windows Adapter V9"),
            ]));
        ...
        Assert.DoesNotContain("Work VPN — 10.8.0.2", combo.Items.Cast<string>());

        showAll.Checked = true;

        Assert.Contains("Work VPN — 10.8.0.2", combo.Items.Cast<string>());
        Assert.False(vm.IsDirty);
    }
    ...
    public void NoisyAdapterSetRendersOnlyTheRealLanUntilShowAllIsSelected()
    {
        SettingsViewModel vm = NewViewModel(
            networkAdapters: new StubNetworkAdapterProvider(
            [
                new NetworkAdapterInfo("Ethernet", "192.168.50.12", "Realtek PCIe GbE", HasIpv4DefaultGateway: true),
                new NetworkAdapterInfo("vEthernet (WSL)", "172.20.16.1", "Hyper-V Virtual Ethernet Adapter"),
                new NetworkAdapterInfo("Work VPN", "10.8.0.2", "TAP-Windows Adapter V9"),
        ...
        Assert.Equal("Auto (recommended) | Ethernet — 192.168.50.12", filtered);
        Assert.Equal(
            "Auto (recommended) | Ethernet — 192.168.50.12 | vEthernet (WSL) — 172.20.16.1 | Work VPN — 10.8.0.2 | Bluetooth Network Connection — 169.254.2.4",
            all);
    }
