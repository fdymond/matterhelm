...
/// the working values into <see cref="HtpcMatterBridge.Config.Current"/>,
/// persists via <see cref="HtpcMatterBridge.Config.Save"/>, then
/// <see cref="HtpcMatterBridge.Config.Reload"/>s so the existing
...
public sealed class SettingsViewModel
{
    ...
    /// <summary>Where the bridge keeps Matter fabric state (BLUEPRINT §2.5) — display-only on the Advanced page.</summary>
    public static string StorageDirDisplay { get; } = Path.Combine(
