...
| S2-2 ✅ (historical; amended by S10-31 and ADR-013/S11-3) | Initial `ActionExecutor`/media keys/CoreAudio/display power; current media route is application-identity-aware focused-first with absolute SMTC fallback | Manual demo: each action works on real media; volume observation fires state updates | M | S0-5 | impl (Fable) |
...
| S4-5 ✅ | Maintainer-reported: DPI overlap fix (PairingWindow rebuilt; all windows audited at 200 %) + overlay volume percentage-bar pill | Screenshots at real 200 % display; pixel-sampled fill assertions; 272 tests + 3 demos green | M | S4-3 | impl (Fable) |
...
## Sprint 5 — telemetry, diagnostics, latency instrumentation (maintainer-directed)
...
## Sprint 6 — deep review & measured optimization (maintainer-directed)
...
## Sprint 7 — key actions, fast taps, MatterHelm rename, OSS setup (maintainer-directed)
...
| S7-3 ✅ (repo renamed to `matterhelm`; npm audit highs fixed; release workflow validates at first v* tag) | OSS-grade project setup: repo → `matterhelm` (remained private at the time), LICENSE, CONTRIBUTING, SECURITY, CoC, issue/PR templates, release workflow (changelog+semver tags), branch/PR conventions | Files in place; release workflow dry-run green | M | S7-2 | impl (Sonnet) + integrator |

## Sprint 8 — trigger mechanics (maintainer-directed)
...
## Sprint 9 — settings UI polish (maintainer-directed)
...
| S9-7 ✅ | Overlay opacity editor → slider (maintainer request): SettingKind.Slider (TrackBar + live "NN %" label, 5/10 steps); descriptor unchanged Get/Set so round-trip tests stand | Settings demo PASS, Overlay page screenshot-verified; app 463 green | S | S9-4 | integrator |
| S9-8 ✅ | "Stop screensaver" fixed (maintainer report: did nothing): ±1 px nudge < scrnsave anti-jitter threshold; replaced with enumerate-and-CloseMainWindow of `.scr` processes (covers self-launched savers SPI never reports; kill fallback for windowless; protected-process exceptions skipped) | Live probe: saver found + gone <1 s via the exact mechanism; app 463 green | S | S8-5 | integrator |

## Sprint 10 — public-launch preparation (maintainer-directed)
...
| S10-18 ✅ | Displays-off sleeps Modern Standby PCs despite the keep-awake hold (maintainer report; kernel events prove MS entry 2 s after acquire): DDC/CI hardware monitor power-off (VCP D6=0x04/0x01) as the primary path — OS never sees screen-off; blanking+hold fallback for non-DDC panels; per-monitor path logging | shipped in 0.4.3; 7 policy-matrix tests | M | S10-13 | integrator |
...
| S10-21 ✅ | mDNS interface adapter dropdown with Auto + "(not detected)" stale state (maintainer request after second-PC wired mis-pin) | shipped in 0.4.3; 66 focused tests | S | — | integrator |
...
| S10-29 ✅ reverted | Attempted timing-based trailing-Off suppression | Reverted by maintainer decision; timing cannot distinguish controller intent from echo | S | S10-28 | integrator |
...
| S10-31 ✅ (historical; amended by ADR-013/S11-3) | Established absolute SMTC verbs after Spotify/YouTube appcommand toggling measurements; current route adds application-identity-aware focused delivery without reintroducing a session toggle | Spotify/YouTube toggle evidence recorded; focused tests | M | S10-30 | integrator |
...
| S11-3 ✅ (mouse macro rejection superseded by S11-5) | Release-blocking S11-2 remediation: application-identity-aware session verification, absolute Play/Pause fallback, protocol-v5 custom edge, Kodi duplicate guard, one route deadline, mouse-capture reconciliation, then-required macro rejection, truth-aligned docs | Kodi-focused/Chrome-session and source-app-identity-swap tests; protocol-to-executor Off/On/repeat/restart/multi-command tests; exact v5 mirror; Release build 0 warnings; targeted app 367 green; bridge 393 green with coverage thresholds under sandbox thread-pool workaround; exact full app/standard Vitest exclusions recorded in the assignment report | L | S11-2 | integrator |
| S11-5 ✅ | Allow mouse movement as a command-sequence step with explicit one-shot absolute semantics and Settings editor support | Parse/save/reload; shared target picker and clamping; no nested/over-cap regression; stop on mouse-step failure; stateless macro path cannot alter retained command restore state; no protocol change | M | S11-3 | integrator |
| S11-6 ✅ | Restore the pre-screensaver foreground window when MatterHelm dismisses a screensaver | Capture precedes Power/custom start; HWND/PID/name validation; minimized restore plus AttachThreadInput fallback; refusal/stale/lifecycle/replace/no-capture tests; honest manual-dismiss limitation; no protocol change | M | S11-5 | integrator |
| S11-7 ✅ shipped in 0.7.1 | Sequence-editor UI consolidation (maintainer request): mouse move is a choice in the single Add… step-type flow; all step types add/edit through SequenceStepDialog and share the standalone target picker | Dropdown indices and every step-type round-trip pinned; sequence limits and one-shot mouse semantics unchanged; clean gates: app 826/826, bridge 393/393; demos overlay/pairing-window/wired PASS | S | S11-5 | integrator |
| S11-8 ✅ shipped in 0.7.1 | Field remediation: screensaver dismissal succeeds independently of best-effort focus restore; restore retries across the transient no-foreground gap; targeted media fallback is same-app only; timed-out appcommands retry once; macro failures use diagnostic reasons | 45 focused tests plus clean gates: app 826/826, bridge 393/393; demos overlay/pairing-window/wired PASS | M | S11-6 | integrator |
| S11-9 ✅ shipped in 0.7.1 | Pairing-window first-show centring follows the final DPI/font-driven preferred size instead of stale pre-layout bounds | First-visible-frame, first-show, late-Shown fallback, and current-screen stage-transition coverage; clean gates: app 826/826, bridge 393/393; demos overlay/pairing-window/wired PASS | S | S11-8 | integrator |
| S11-10 ✅ shipped in 0.7.1 | Pre-release remediation: bridge-disable macro/focus lifecycle, `MouseTargetPicker.cs` rename, test-honesty coverage, and documentation correction so Win32 error 5 is only a likely UIPI signal | Clean gates: app 826/826, bridge 393/393; app line coverage about 58% (45% threshold); demos overlay/pairing-window/wired PASS | S | S11-7..9 | integrator |

## Sprint 12 — public launch

| ID | Story | Acceptance criteria | Size | Deps | Agent |
|---|---|---|---|---|---|
| S12-1 ✅ | Supply-chain and CI hardening for public launch | SHA-pinned Actions; least-privilege permissions; CodeQL (JS/TS + C#), dependency review, Scorecard, provenance attestations; npm/csproj metadata; Vitest 4.1.11 audit remediation; exact local `postject` | M | S11-10 | integrator |
| S12-2 ✅ | Documentation truth pass for the public switch | v0.7.1 remains current; protocol v5, private-repository preparation, release evidence, public docs, and local links reconciled | M | S11-10 | integrator |
| S12-3 ✅ | Read-only whole-repository code-review pass; confirmed fixes follow separately | Review findings were evidence-backed and triaged into RB-1..RB-6, SF-2..SF-10, M-1..M-7, and P-10..P-12 before remediation | M | S12-1..2 | integrator |
| S12-4 ✅ | Public-switch code review remediation and BridgeHost/MediaKeys decomposition | RB-1..RB-6, SF-2..SF-10, M-1..M-7, and P-10..P-12 remediated; ADR-014 records trust-boundary, bounded-pipeline, reset, packaging, updater, and structure decisions | L | S12-3 | integrator |

## Deferred work

| ID | Story | Acceptance criteria | Size | Deps | Agent |
|---|---|---|---|---|---|
| ~~P-6~~ ✅ shipped under S12-4 | Pin `postject` exactly and invoke the local binary for SEA release builds | `bridge/package.json` + lockfile pin `1.0.0-alpha.6`; `build.ps1` invokes the local package binary | S | — | integrator |

## Proposed (from agent reports, integrator-triaged)

| ID | Proposal | Gate before implementation |
|---|---|---|
| SF-1 | Signed release manifest and/or Authenticode | Maintainer decision: `SHA256SUMS.txt` from the same release is not an independent authenticity root |
| M-8 | Pool IPC frame buffers | Benchmark first; retain the simpler bounded allocation path unless pooling is measurably useful |
| M-9 | Split `SettingsWindow` into smaller collaborators | Add UI tests around the seams before restructuring the WinForms shell |
| M-10 | Add direct `SystemVolume` tests | Build a controlled CoreAudio boundary/fake; the current test covers only reacquire scheduling |
| M-11 | Evaluate matter.js 0.17.9 | Re-run ADR-010 Windows mDNS/scoped-IPv6 evaluation before changing the 0.17.7 pin |
| M-12 | Keep the seed-persistence warning when config saves are blocked | Documentation completed in W9; retain it as a regression-review requirement wherever rejection/pairing guidance changes |
| M-13 | Preserve a non-QR line prefix when replacing a long block-art run | Cosmetic only; add focused diagnostics tests before changing whole-line `<qr-art>` replacement |
| M-14 | Emit one `ipc.drop` WARN during a remote-close handshake | Cosmetic only; prove one-per-disconnected-episode behavior without adding reconnect noise |
| M-15 | Review generated `THIRD-PARTY-NOTICES.txt` coverage before the public tag | Compare the exact packaged dependency/runtime closure with generated notice sections and block the tag on omissions |

- ~~**P-10**~~ ✅ shipped under S12-4: factory reset now atomically renames the
  live Matter directory before best-effort deletion; rename failure touches
  nothing and cleanup failure records the staged residue path.
- ~~**P-11**~~ ✅ shipped under S12-4: both updater modes consume
  `sidecar-layout.json`; installer and portable upgrades remove the other
  sidecar layout's known files without deleting unrelated portable files.
- ~~**P-12**~~ ✅ shipped under S12-4: diagnostics now sanitise config by
  default and redact commissioning credentials, machine/user identity, profile
  paths, and IP literals from every bundled log/metrics line.
- **P-7** (deferred by maintainer 2026-08-30, feasibility already established): make a
...
- **P-8** (CLOSED 2026-08-30 — will not implement): system-wide cursor HIDING.
  The park half shipped in 0.6.0 as the mouse-move command category. The
  hiding half (SetSystemCursor with a blank cursor) was implemented and then
  REMOVED at the maintainer's direction: "do not hide the cursor as this as
  described can be breaking". Rationale stands on the record — a crash, kill
  or failed restore while the system cursors are blanked leaves the machine
  with no visible pointer, and the mitigations (restore-on-exit, a startup
  marker file, a tray escape hatch) reduce but do not eliminate that risk.
  Measured constraint for anyone revisiting this: Windows CLAMPS the cursor to
  the virtual desktop, so a pointer cannot be parked off-screen — requested
  (5000,5000) landed at (1600,1000) and (-500,-500) at (0,0) on a 1600x1000
  desktop. Parking in a corner is therefore the only non-invasive option; a
  top corner keeps it clear of the control bar most players draw at the bottom.
...
- ~~**P-5**~~ ✅ shipped under S12-4: supervisor/test logging is injected or
  redirected to test-owned temporary directories, so tests do not create
  `%APPDATA%\MatterHelm` state.
