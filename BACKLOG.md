...
| S11-3 ✅ (mouse macro rejection superseded by S11-5) | Release-blocking S11-2 remediation: ownership-aware session verification, absolute Play/Pause fallback, protocol-v5 custom edge, Kodi duplicate guard, one route deadline, mouse-capture reconciliation, then-required macro rejection, truth-aligned docs | Kodi-focused/Chrome-session and owner-swap tests; protocol-to-executor Off/On/repeat/restart/multi-command tests; exact v5 mirror; Release build 0 warnings; targeted app 367 green; bridge 393 green with coverage thresholds under sandbox thread-pool workaround; exact full app/standard Vitest exclusions recorded in the assignment report | L | S11-2 | integrator |
...
| S11-7 🔄 partial | Sequence-editor UI consolidation (owner request): mouse move is a choice in the existing Add… step-type dropdown; all step types add/edit through SequenceStepDialog and share the standalone target picker | Dropdown indices and every step-type round-trip pinned by tests; existing sequence limits and one-shot mouse semantics unchanged; clean-runner app/bridge gates pending after documented sandbox exclusions | S | S11-5 | Codex |
| S11-8 🔄 partial | Field remediation: screensaver dismissal succeeds independently of best-effort focus restore; restore retries across the transient no-foreground gap; targeted media fallback is same-owner only; timed-out appcommands retry once; macro failures use diagnostic reasons | Implementation + 45 focused tests + Release build green; sandbox app remainder 763/763 and alternate bridge runner 393/393, but clean standard full app/bridge gates remain required after `HttpListener` invalid-handle and Vitest `spawn EPERM` exclusions | M | S11-6 | Codex |
| S11-9 🔄 partial | Pairing-window first-show centring follows the final DPI/font-driven preferred size instead of stale pre-layout bounds | `FirstVisibleFrameAlreadyUsesTheSettledCenteredBounds`, `FirstShowRemainsCentered`, late-`Shown` fallback, and current-screen stage-transition coverage; clean standard full app/bridge gates remain pending | S | S11-8 | Codex |
...
- **P-8** (CLOSED 2026-08-30 — will not implement): system-wide cursor HIDING.
  ...
  REMOVED at the owner's direction: "do not hide the cursor as this as
